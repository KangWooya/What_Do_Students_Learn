...
import bisect, warnings  # 파일 상단에 추가
...
class DatasetWrapper(Dataset):
    ...
    def __init__(self, dataset, indices=None):
        ...
        # torchvision 0.2.x 호환 대신: 존재 확인 기반으로 안전하게 설정
        if isinstance(self.base_dataset, datasets.ImageFolder):
            # ImageFolder는 imgs[(path, class)] → class 인덱스를 targets로 통일
            if not hasattr(self.base_dataset, "targets"):
                self.base_dataset.targets = [s[1] for s in self.base_dataset.imgs]
        else:
            # CIFAR 계열: 최신은 targets가 이미 있음
            if hasattr(self.base_dataset, "targets"):
                pass  # 그대로 사용
            elif hasattr(self.base_dataset, "train_labels"):
                self.base_dataset.targets = self.base_dataset.train_labels
            elif hasattr(self.base_dataset, "test_labels"):
                self.base_dataset.targets = self.base_dataset.test_labels
            else:
                raise AttributeError(
                    "Unsupported dataset: no 'targets', 'train_labels', or 'test_labels' found."
                )
...
class ConcatWrapper(Dataset): # TODO: Naming
    ...
    def get_class(self, idx):
        ...
        tt = getattr(self.datasets[dataset_idx].base_dataset, "target_transform", None)
        return tt(true_class) if tt else true_class
