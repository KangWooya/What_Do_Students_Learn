...
__all__ = ['ResNet', 'resnet10', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
           'resnet152', 'resnext50_32x4d', 'resnext101_32x8d', 'CIFAR_ResNet', 'CIFAR_ResNet18', 'CIFAR_ResNet34', 'CIFAR_ResNet50','CIFAR_ResNet10']
...
class PreActBottleneck(nn.Module):
    """Pre-activation version of Bottleneck (for CIFAR ResNet-50)."""
    expansion = 4
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.conv1 = conv1x1(in_planes, planes)              # 1x1 reduce

        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = conv3x3(planes, planes, stride=stride)  # 3x3 spatial (downsample here if stride=2)

        self.bn3 = nn.BatchNorm2d(planes)
        self.conv3 = conv1x1(planes, planes * self.expansion)  # 1x1 expand
    ...
    def forward(self, x):
        out = F.relu(self.bn1(x))
        shortcut = self.shortcut(out)  # NOTE: preact uses BN/ReLU before shortcut projection
...
def CIFAR_ResNet50(pretrained=False, **kwargs):
    """
    CIFAR-style ResNet-50 using Pre-activation Bottleneck.
    Stages: [3, 4, 6, 3], downsample at layer2/3/4 (stride=2) as in CIFAR_ResNet.
    """
    return CIFAR_ResNet(PreActBottleneck, [3, 4, 6, 3], **kwargs)
